        ...
        Api.view(object, function (data) {
          ...
          if (menu) {
            menu.forEach(function (m) {
              ...
              m.title = spl[0].trim();
              if (spl[1]) m.subtitle = Lampa.Utils.capitalizeFirstLetter(spl[1].trim().replace(/all/i, 'Р›СЋР±РѕР№'));

              if (m.submenu) {
                m.submenu.forEach(function (s) {
                  s.title = Lampa.Utils.capitalizeFirstLetter(s.title.trim().replace(/all/i, 'Р›СЋР±РѕР№'));
                });
              }
            });
          }
